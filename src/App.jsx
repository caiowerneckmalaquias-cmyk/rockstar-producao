...
const initialMinimos = {
  BTCV010: { Preto: Object.fromEntries(sizes.map((s) => [s, { pa: s <= 38 ? 12 : 8, prod: 24 }])) },
  TNCV010: { Branco: Object.fromEntries(sizes.map((s) => [s, { pa: s <= 39 ? 12 : 8, prod: 24 }])) },
  CRVTNCV: { "All Black": Object.fromEntries(sizes.map((s) => [s, { pa: 8, prod: 24 }])) },
};
...
const [importMode, setImportMode] = useState("replace");
  ...
  const executeImport = async () => {
    ...
    setRows((current) => {
      ...
      const novos = parsed.filter((item) => !usados.has(`${normalizeKey(item.ref)}__${normalizeKey(item.cor)}`));
